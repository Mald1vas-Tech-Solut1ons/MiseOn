...
### 20-K. Checkout e fechamento pelo garçom — 10/09/2026

- A função `cartao-pagar` publicada continua usando `EFI_COBRANCAS_*`, atualmente vinculada à conta Efí **PF**, conforme a decisão temporária registrada em 20-J. Nesta correção não foram trocados secrets, recebedor, `payee_code` ou ambiente.
- A tentativa registrada às 15:37 UTC (12:37 BRT) recebeu da Efí o código `4600222` (recebedor e cliente iguais). O checkout reutilizava nome/CPF de titular salvo mesmo quando o número do cartão era trocado. A interface agora oferece “Trocar titular” e mantém o drawer montado após recusa, preservando carrinho, contato, endereço, cupom e forma de pagamento.
- O endereço padrão já existia no banco, mas `EnderecoMixin` copiava `valorInicial` apenas antes da consulta assíncrona terminar. O componente agora sincroniza os campos quando o endereço salvo chega.
- Por decisão explícita do PO, o papel `garcom` pode receber presencialmente e fechar uma comanda buffet. A RPC continua sendo a única fronteira de escrita: registra pagamento, finaliza pedido, fecha a comanda e grava `fechada_por` na mesma transação.
- Segurança de cartão: somente nome e CPF do titular podem ser lembrados. Número, validade e CVV não são persistidos; o número é tokenizado no navegador.
- Provas locais: testes focados 4/4, `tsc --noEmit`, ESLint e build de produção aprovados. A RPC foi executada com papel real de garçom dentro de transação e todas as asserções passaram; a transação foi revertida para não deixar dados de teste.
...
## Execução de 09/09 (parte 7) — engenharia de produção completa

**Objetivo:** fechar o ciclo da produção com o rigor do food service: o que sai do estoque, o que entra no prato, o que a cozinha gasta para transformar e o que ela realmente entrega.

**Modo de Preparo.** A OS deixou de ser um cronômetro solto e virou execução em tela cheia: mise en place conferida item a item (não libera o preparo sem conferir), roteiro passo a passo com cronômetro por etapa e bipe, cronômetro global da OS e conclusão gamificada até a etiqueta. O roteiro mora em `insumos.modo_preparo` (jsonb), com marcação de etapa em fogo/forno.

**Custo de conversão.** Gás e mão de obra passam a ser apurados pelo tempo **MEDIDO** na bancada: gás = `botijão ÷ peso × consumo kg/h × minutos de chama`; mão de obra = `valor hora × tempo total`. `tempo_origem` distingue MEDIDO de ESTIMADO. **Decisão contábil:** ficam registrados na OS e **não** são capitalizados no lote, porque `configuracoes_custo` já rateia esses custos no preço de venda — somar de novo contaria o mesmo real duas vezes.

**Fator de correção (bruto × líquido).** `fichas_preparos.quantidade` é o BRUTO na unidade de estoque (a loja pagou pela casca) e é o que o custo e o CMV contam. `quantidade_liquida` é o que entra no preparo e passa a ser a base da nutrição. Medido: 748 g contra 910 g — 22% de erro de caloria eliminado. `rendimento_pct_aplicado` e `rendimento_origem` são snapshot; `quantidade_informada`/`unidade_informada` preservam a intenção ("5 un" com o banco guardando 0,600 kg).

**Domínio da técnica culinária, como dado.** `tecnicas_culinarias` (25 técnicas), `tecnicas_rendimento_ingrediente` (73 fatores casados pelo nome), `tecnicas_rendimento_categoria` e `insumos_tecnica_rendimento` (o que a loja mediu, com média móvel por amostras). `fn_rendimento_tecnica` resolve na ordem MEDIDO_LOJA > REFERENCIA_INGREDIENTE > REFERENCIA_CATEGORIA e devolve **nulo** quando não tem base — o sistema domina a técnica e não chuta.

**Perda e ganho de cocção.** O teto de 100% cai nas tabelas de referência: cocção seca perde (frango assado 70%), grão hidrata (arroz 250%). Na OS, a equipe pesa o lote pronto e **entra no estoque o que existe de fato** — dizer que entraram 3 L quando saíram 2,4 é estoque fantasma. `fn_rendimento_real_preparo` devolve o histórico das últimas 20 produções, e a ficha mostra ao lojista o que a cozinha dele realmente entrega.

**Procedência do item.** O seletor separa Matéria-prima, Preparos da casa, Comprado pronto e Embalagens/descartáveis, cada grupo explicando o que significa. Item de revenda não oferece técnica de limpeza — queijo do saquinho já vem ralado; quem rala o parmesão da geladeira faz uma ficha própria, que vira preparo com custo de produção e lote rastreável.

**Classificação inteligente e que aprende.** `fn_classificar_insumo` recebia `p_nome` e nunca usava. Agora a ordem é NCM (verdade fiscal) > léxico do nome > catálogo de categoria > Outros. `classificacao_lexico` nasceu com limpeza, higiene, EPI e manutenção, e ganhou embalagem e descartáveis. A Edge Function da NF-e passou a receber o NCM de cada item, a carregar a taxonomia do banco (`fn_categorias_classificacao`) em vez de uma lista fixa desatualizada, e a **promover ao léxico** o que classifica com confiança (`fn_aprender_termo_lexico`): da segunda nota em diante o mesmo item resolve sem IA, de graça e auditável em `vw_lexico_aprendido`. Comida nunca é aprendida — o erro tem que ser barato.

**Defeitos corrigidos no caminho:** a `fn_produzir_preparo` de produção inseria direto em `movimentacoes_estoque` (custo nulo, PEPS sem consumir lote, preparo valendo zero); a migration local declarava `(uuid, integer)` contra a `(uuid, numeric)` existente, criando sobrecarga ambígua; `rendimento_porcoes` era `integer` e arredondava meio litro em silêncio; `classificacao_categorias` apontava para tipos `OPERACIONAL`/`OUTROS` inexistentes em `tipos_item`, e o INNER JOIN da view `itens` fazia esses insumos **desaparecerem do sistema**; o passo 8 do tour apontava para uma aba extinta; etiqueta sem validade saía em branco.

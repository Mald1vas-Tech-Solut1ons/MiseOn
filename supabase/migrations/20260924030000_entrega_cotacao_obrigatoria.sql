-- Entrega profissional, etapa 2: cotação obrigatória e fim do modelo por bairro.
--
-- O checkout que manda cotacao_id está no ar desde feb6610 (24/09/2026). A
-- partir daqui o pedido de entrega sem cotação é recusado: a coordenada do
-- navegador deixa de ter qualquer efeito.
--
-- Sai também o que só existia para o modelo antigo:
--   taxas_entrega              lista de bairros; o checkout ignorava e o chat
--                              lia — preço diferente em cada canal. Backup em
--                              backups/taxas_entrega-2026-09-24.json (7 linhas).
--   fn_taxa_entrega_calculada  cálculo com lat/lng do cliente; estava exposta
--                              a anon em /rest/v1/rpc.
...
begin
  ...
  if v_tipo = 'DELIVERY' then
    ...
    -- Entrega só com cotação do servidor (edge function entrega-cotar).
    if nullif(p_payload->>'cotacao_id','') is null then
      raise exception 'Atualize a pagina para calcular a entrega pelo endereco.';
    end if;
    begin
      -- Distância medida pelo SERVIDOR (edge function entrega-cotar). A
      -- coordenada do navegador não entra na conta.
      select * into v_cot from entrega_cotacoes
       where id = (p_payload->>'cotacao_id')::uuid and loja_id = v_loja;
      if v_cot.id is null or v_cot.expira_em < now() then
        raise exception 'A cotacao da entrega expirou. Confira o endereco e tente de novo.';
      end if;
      -- A cotação vale para o endereço cotado: trocar o endereço depois de
      -- cotar um mais perto não pode baratear a entrega.
      if regexp_replace(coalesce(v_end->>'cep',''), '[^0-9]', '', 'g') <> coalesce(v_cot.cep,'')
         or upper(btrim(coalesce(nullif(v_end->>'numero',''), 'SN'))) <> upper(coalesce(v_cot.numero,'SN')) then
        raise exception 'O endereco mudou depois do calculo da entrega. Confira o endereco e tente de novo.';
      end if;

      v_entrega := public.fn_entrega_regra(v_loja, v_cot.distancia_km, coalesce(v_subtotal_srv, 0));
      if not coalesce((v_entrega->>'atende')::boolean, false) then
        raise exception '%', coalesce(v_entrega->>'mensagem', 'Endereco fora da area de entrega.');
      end if;

      update pedidos
         set taxa_entrega = coalesce((v_entrega->>'taxa')::numeric, 0),
             distancia_km = v_cot.distancia_km,
             lat = v_cot.lat,
             lng = v_cot.lng
       where id = v_pedido;
    end;
...
drop function if exists public.fn_taxa_entrega_calculada(uuid, numeric, numeric, text, numeric);
drop table if exists public.taxas_entrega;

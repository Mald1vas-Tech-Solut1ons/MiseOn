-- A importação de nota atende às três situações que o food service vive, e que
-- antes eram tratadas como se fossem a mesma:
--
--   1. COMPRA DO DIA — o fluxo normal. Soma ao saldo, entra na fila do PEPS
--      com a data da nota, e o perecível leva validade.
--
--   2. NOTA ANTIGA JÁ CONSUMIDA — o lojista acha um cupom de três semanas
--      atrás na gaveta. Somar aquilo ao saldo criaria estoque que não existe
--      na prateleira: a mercadoria já virou prato vendido. Mas o CUSTO daquela
--      compra é história valiosa (é ela que diz se o fornecedor aumentou).
--      Modo HISTORICO: registra preço e de-para, não mexe no saldo.
--
--   3. MIGRAÇÃO PARA O SISTEMA — o cliente novo chega com a prateleira cheia e
--      um maço de notas. Ele lança as notas recentes (modo 1, que traz custo
--      real) e conta o resto no inventário. O que não pode é lançar seis meses
--      de nota e ficar com um estoque três vezes maior que a prateleira.
--
-- Cada item pode trazer `vence_em`, `fabricado_em` e `lote_fornecedor` — o que
-- sustenta alerta de vencimento, PVPS e recall de fornecedor.
...
BEGIN
  ...
  LOOP
    ...
    IF v_vence IS NOT NULL THEN
      ...
      -- Item que já chegou vencido não é bloqueado: o lojista pode estar
      -- lançando compra antiga de propósito. Mas o número volta na resposta
      -- para a tela poder dizer isso em voz alta.
      IF v_vence < CURRENT_DATE THEN v_vencidos := v_vencidos + 1; END IF;
    END IF;

    -- Modo HISTORICO não mexe no saldo: a mercadoria daquela nota já foi
    -- consumida. O que interessa dela é o preço, que fica no de-para e no
    -- último custo do insumo.
    IF v_somar THEN
...
DROP FUNCTION IF EXISTS public.fn_importar_nfce(UUID, TEXT, TEXT, JSONB, BOOLEAN, TIMESTAMPTZ);

REVOKE ALL ON FUNCTION public.fn_importar_nfce(UUID, TEXT, TEXT, JSONB, BOOLEAN, TIMESTAMPTZ, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_importar_nfce(UUID, TEXT, TEXT, JSONB, BOOLEAN, TIMESTAMPTZ, TEXT) TO authenticated;

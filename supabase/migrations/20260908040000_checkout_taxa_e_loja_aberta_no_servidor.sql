-- ============================================================================
-- SPRINT 6: CHECKOUT — A TAXA E A PORTA PASSAM A SER DO SERVIDOR
--
-- Reescrita de fn_criar_pedido_completo SOBRE a definição que está em
-- produção (lida com pg_get_functiondef antes de tocar — ver o histórico de
-- drift em 20260721100000_versionar_drift_producao.sql). Nada do corpo antigo
-- foi removido: entram apenas dois guardas e a derivação da taxa.
--
-- 1. LOJA FECHADA — antes só o browser barrava (CheckoutDrawer.tsx: `if
--    (!aberta && quando === 'AGORA')`). A regra é mantida igual, inclusive a
--    exceção: pedido AGENDADO continua entrando com a loja fechada.
--
-- 2. TAXA DE ENTREGA — antes `p_payload->>'taxa_entrega'` era gravado como
--    veio. Agora nasce 0 e é derivada por fn_taxa_entrega_calculada sobre o
--    subtotal JÁ recalculado pelo servidor (o frete grátis depende dele).
--    Recalcula-se duas vezes de propósito, o mesmo padrão que o cashback
--    logo abaixo já usava.
--
-- Endereço fora da área de entrega passa a derrubar o pedido: antes o browser
-- mostrava "Fora da área" e um POST direto ignorava isso.
-- ============================================================================

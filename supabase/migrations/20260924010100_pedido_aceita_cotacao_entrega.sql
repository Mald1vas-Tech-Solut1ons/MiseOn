-- Entrega profissional, etapa 1: o pedido aceita a cotação do servidor.
-- Base: pg_get_functiondef de produção em 23/09/2026 (só o bloco DELIVERY muda).
...
begin
  ...
  if v_tipo = 'DELIVERY' then
    ...
    if nullif(p_payload->>'cotacao_id','') is not null then
      ...
      -- LEGADO (etapa 1): checkout antigo, coordenada do navegador. A etapa 2
      -- remove este ramo e passa a exigir cotação.

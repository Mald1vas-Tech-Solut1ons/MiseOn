  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-2 border-b border-gray-200 bg-white px-3 py-2 dark:border-gray-800 dark:bg-gray-900 sm:px-4 sm:py-2.5">
      ...
      <div className="flex gap-1.5">
        {turno ? (
          <>
            <button onClick={() => setModalCaixa('SANGRIA')} title={tDynamic('Sangria')} aria-label={tDynamic('Sangria')} className="flex items-center gap-1 rounded-xl border border-gray-200 px-3 py-1.5 text-xs font-bold text-gray-600 dark:border-gray-700 dark:text-gray-300"><ArrowDownCircle size={13} /> <span className="hidden sm:inline">{tDynamic('Sangria')}</span></button>
            <button onClick={() => setModalCaixa('REFORCO')} title={tDynamic('Reforço')} aria-label={tDynamic('Reforço')} className="flex items-center gap-1 rounded-xl border border-gray-200 px-3 py-1.5 text-xs font-bold text-gray-600 dark:border-gray-700 dark:text-gray-300"><ArrowUpCircle size={13} /> <span className="hidden sm:inline">{tDynamic('Reforço')}</span></button>
            <button onClick={() => { setValorCaixa(''); setModalCaixa('FECHAR'); }} title={tDynamic('Fechar caixa')} aria-label={tDynamic('Fechar caixa')} className="flex items-center gap-1 rounded-xl bg-gray-900 px-3 py-1.5 text-xs font-bold text-white dark:bg-gray-700"><Lock size={13} /> <span className="hidden sm:inline">{tDynamic('Fechar caixa')}</span></button>
          </>
        ) : (
          <button onClick={() => setModalCaixa('ABRIR')} className="flex items-center gap-1 rounded-xl bg-[var(--cor-primaria)] px-4 py-1.5 text-xs font-bold text-white"><Unlock size={13} /> {tDynamic('Abrir caixa')}</button>
        )}
      </div>
    </div>
  );
